...
func init() {
	ctx := gctx.New()
	realm, _ := g.Cfg().Get(ctx, "jwt.realm")
	jwtkeyStr, _ := g.Cfg().Get(ctx, "jwt.key")
	jwtkey := []byte(jwtkeyStr.String())
...
func IdentityHandler(ctx context.Context) interface{} {
	...
	return claims[sAuth.IdentityKey]
}
...
func Authenticator(ctx context.Context) (interface{}, error) {
	...
	user, err := User().Login(ctx, in.Username, in.Passwd)
	if err != nil {
		return "", err
	}
	return user, nil
